...
function buildAgreementDoc(c, logoBuffer) {
  ...
  const agreementChildren = [
    ...
    sectionHeading('13. WAREHOUSING & LOGISTICS'),
    clauseTitle('13.1  Fulfilment from Vendor Warehouse'), body('All orders forwarded to Party A through Party B\'s sales channels shall be fulfilled directly from Party A\'s own warehouse. Party A shall be solely responsible for the storage, packaging, and timely dispatch of all such orders.'),
    ...
    clauseTitle('15.3  Effect of Termination'), body('Upon termination of this Agreement: (a) all outstanding payments shall be settled within 14 business days, subject to deduction of any applicable penalties, reserves, or liabilities; (b) Party B shall delist all of Party A\'s products from its platform; and (c) the provisions relating to confidentiality, intellectual property, dispute resolution, limitation of liability, and return/quality obligations shall survive termination and remain in full force and effect.'),
    spacer(20), hr(),
...
Rules:
- CRITICAL: call the relevant tool again on EVERY turn that needs order/product/delay data — even if you already looked up the exact same order or search term earlier in this conversation, even if the customer is just confirming ("yes", "yes status", "ok check it") rather than asking something new. Never answer from memory of an earlier tool result in this conversation. If your reply mentions a "card below" or any specific status/price/stage, that information must come from a tool call you just made in THIS turn — if you didn't call a tool this turn, you have no card to refer to, so don't claim one is shown.
...
// ── Handle replies from the vendor WhatsApp number ────────────────────────
// Supports two formats (case-insensitive):
//   TRACK <order> <AWB> <courier>       → saves tracking to order_vendor_stage
//   DELAY <order> <reason> | ETA <date> → saves delay_reason + resolution date
...
// ── Vendor delay reason + ETA from DB ─────────────────────────────────────
async function waDelayReason(shopifyOrderId) {
  ...
  if (!vs?.delay_reason) return null;
